                ...
                for action, prob in enumerate(action_probs):
                    if prob > self.threshold:  # Only plot if probability is significant
                        ...
                        ax.arrow(
                            ...
                            fc=self.color,
                            ec=self.color,
                            alpha=min(1.0, prob * 1.5),  # Higher probability = more opaque
                            length_includes_head=True,
                        )
                        
                        # Optionally show probability text
                        if self.show_text and prob >= 0.1:  # Only show for significant probabilities
                            # Position the text near the arrow
                            text_pos = (col + dx * 0.6, row + dy * 0.6)
                            ax.text(
                                text_pos[0], 
                                text_pos[1], 
                                f"{prob:.2f}", 
